...
export function SenateRegionList({ regions, factions, autoSeats, autoAssign }: SenateRegionListProps) {
  ...
  return (
    <Panel
      title={t('regions').toUpperCase()}
      subtitle={`${totalSeats} ${t('seats_total')}`}
      bodyClassName="no-scroll"
      actions={
        autoAssign && autoTotal > 0
          ? <span className="senate-auto-badge">{autoTotal} {t('auto_assigned')}</span>
          : undefined
      }
    >
      {regions.filter(r => (r.seatings || 0) > 0).length === 0 ? (
        <EmptyState>{t('no_regions_with_seats')}</EmptyState>
      ) : (
              ...
              <div key={region.id} className="senate-region-row">
                <div className="senate-region-main">
                  ...
                  {autoAssign && isDominant && seats > 0 && (
                    <span className="senate-auto-dot" style={{ color: entries[0].color }} title={`${t('auto_assigned_to')} ${entries[0].label}`}>◈</span>
                  )}
                </div>
                {entries.length > 0 && (
                  <DominanceBar
